...
class Asset(object):
    """
    初始化函数：定义强制填写的字段包含sn','asset_id','asset_type'，以及返回信息格式等。
    """
    ...
    def response_msg(self,msg_type,key,msg):
        if self.response.has_key(msg_type):
            ...
            raise ValueError
    """
    强制性检查，检查客户端提交上来的数据，如果没有SN将返回错误信息。
    """
    def mandatory_check(self,data,only_check_sn=False):
        ...
        try:
            ...
            if not only_check_sn: #默认是根据asset_id 获取资产数据对象
            ...
            return False
    """
    定义获取资产ID函数，当客户端提交数据后，获取数据判断数据是否存在，并且强制性检查资产ID，如不存在的话返会错误信息。
    """
    def get_asset_id_by_sn(self):
        '''When the client first time reports it's data to Server,it doesn't know it's asset id yet,so it will come to the server asks for the asset id first,then report the data again  '''
        ...
        return response
    """
    定义新资产临时存放区域，对于客户端汇报上来未审批入库的新资产将放入临时区域，以待审批。
    """
    def save_new_asset_to_approval_zone(self):
        ...
        return True
    """
    定义数据的合法性，对于客户端汇报上来的数据需要强制性检查，通过后才会将干净数据传入下阶段，否则给予错误提示信息。
    """
    ...
    """
    定义私有新资产函数方法，判断数据中是否包含资产类型。
    """
    def __is_new_asset(self):
        if not hasattr(self.asset_obj, self.clean_data['asset_type']):#new asset
            return True #不恶心  self.clean_data['asset_type'] = server
        else:
            return False
    """
    定义数据入库方法，新增资产入库写入以及更新。
    """
    ...
    def __verify_field(self,data_set,field_key,data_type,required=True):
       ...
       :param data_set:  self.clean_data
       :param field_key:  model
       :param data_type:  str
       :param required:
       :return:
       '''
       field_val = data_set.get(field_key) #dell
       if field_val: #not None
            try:
    ...
    def create_asset(self):
        func = getattr(self,'_create_%s' % self.clean_data['asset_type']) #server
        create_obj =func()
    ##更新资产函数
    def update_asset(self):
        func = getattr(self,'_update_%s' % self.clean_data['asset_type'])
        create_obj =func()
    ##更新服务端（数据库）中数据库中网卡，磁盘，内存，cpu等各项要素
        ...
        for field in fields_from_db:
            ...
            if val_from_data_source:
                ...
                if val_from_db == val_from_data_source:# this field haven't changed since last update
                    ...
                    db_field_obj.save_form_data(model_obj, val_from_data_source)
